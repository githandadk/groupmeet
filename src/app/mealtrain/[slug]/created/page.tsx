  ...
  if (!signup) {
    return (
      <main className="min-h-screen flex items-center justify-center px-4">
        <p className="text-gray-500">Meal train not found</p>
      </main>
    );
  }

  const appUrl = typeof window !== 'undefined' ? window.location.origin : '';
  const shareUrl = `${appUrl}/mealtrain/${slug}`;
  const adminUrl = `${appUrl}/mealtrain/${slug}/admin#token=${adminToken}`;
...
export default function MealTrainCreatedPage() {
  return (
    <Suspense fallback={
      <main className="min-h-screen flex items-center justify-center">
        <p className="text-gray-400">Loading...</p>
